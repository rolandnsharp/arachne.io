...
function init() {
  ...
  //   . N nodes
  //   . E edges
  var i, N = hashArray.length, E = hashArray.length;
 
  for(i = 0; i < N; i++){
  	for (j = 0; j< hashArray[i].length; j++){
  	
  	if (j!==0){	
    sigInst.addNode('n'+String(i)+","+String(j),{
      'x': Math.random(),
      'y': Math.random(),
      'label': hashArray[i][j],
 ...
 for(i = 0; i < N; i++){
  	for (j = 0; j< hashArray[i].length; j++){
  sigInst.addEdge(i+"edge"+j,'n'+String(i)+","+String(0),'n'+String(i)+","+String((hashArray[i].length-1)-j));
  ...
  (function(){
    ...
    function showNodeInfo(event) {
      ...
      iNumber = nodeNString.substring(1,nodeNString.length).split(",");
      console.log(iNumber);
      document.getElementById("query").value = hashArray[iNumber[0]][iNumber[1]];
      $("#submit_id").submit();
    }
